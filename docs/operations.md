...
> 불가능해집니다.
>
> 사이드카는 **지우지 않고 옆으로 치웠다가**(`.replacing`) 교체가 끝난 뒤에 지웁니다.
> 마지막 `os.replace` 도 실패할 수 있기 때문입니다. 실패하면 치워 둔 것을 제자리로
> 되돌리고 멈춥니다. "실패하면 보드와 사이드카는 그대로" 가 이 도구의 계약이고,
> 복구 도구가 데이터를 잃는 것은 고치려던 것보다 나쁩니다.
>
