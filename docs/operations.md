...
**안전** — PR 에 악의적인 `postinstall` 스크립트가 들어 있으면 그대로 실행됩니다.
사내 레포만 다루는 동안에는 위험이 낮지만, 외부 기여를 받기 시작하면 실제 문제가 됩니다.

**정리 비용** — 워크트리가 남기 때문에 회수 스크립트가 필요합니다. 컨테이너로 격리하면
작업이 끝나는 순간 통째로 사라져 청소 자체가 불필요해집니다.

Hermes 는 `terminal.backend` 로 컨테이너 실행을 지원합니다. 다만 도커 소켓 접근 권한이
필요하고, 빌드 캐시가 매번 날아가 리뷰 시간이 늘어납니다. 그래서 지금은 미루고,
외부 기여를 받는 시점을 전환 기준으로 잡습니다.

### 그 밖에

- 칸반에 `blocked` 로 남은 태스크를 일정 기간 뒤 자동 아카이브하는 정책
- 리뷰 판정 추이를 대시보드로 (지표는 이미 `hermes-metrics.py` 가 내보내는 중)
