...
| 칸반 보드 SQLite 손상 | 리뷰 파이프라인 **전체** 정지, 35분간 아무도 인지 못 함 |

공통점은 **아무도 몰랐다**는 것입니다. 그래서 `ops/` 에 있는 것은 기능이 아니라 **지켜보는 장치**입니다.

| 스크립트 | 주기 | 하는 일 |
|---|---|---|
| `hermes-watchdog.py` | 2분 | 게이트웨이가 실제로 응답 가능한지 점검, 필요하면 재시작하고 Slack 보고. Slack 소켓 실패는 **옛 세션이 남긴 것과 마지막 연결 성공 이전 것을 빼고** 셉니다. **칸반 보드 무결성**도 함께 보는데, 이것만 재시작 대상이 아닙니다 — 재시작해도 보드는 그대로 깨져 있고 Slack 세션만 끊깁니다 |
...
| `hermes-pr-notifier.py` | 상주 (20초 주기) | 끝난 리뷰를 판정·링크와 함께 Slack 으로 알림 |
...
| `hermes-review-audit.py` | 매일 09:00 | 리뷰가 계약대로 동작했는지 검사하고 오탐 신고(👍/👎)를 모읍니다. GitHub 에는 읽기만 합니다. **월요일에만** 주간 요약을 Slack 으로 보냅니다 |
| `hermes-kanban-restore.py` | 수동 | 손상된 칸반 보드를 검증된 복구본으로 되돌립니다. 워치독이 손상을 감지하면 복구본을 만들어 두고 이 명령을 알립니다 |

표준 라이브러리만 씁니다. 추가 의존성이 없습니다.

`apply-local-patches.py` 와 `hermes-kanban-restore.py` 는 launchd 에 등록하지
않습니다. 사람이 판단해서 실행하는 도구입니다.

감사는 처음에 수동으로 두었습니다. 아무도 보지 않는 잡을 늘리는 것이 이
시스템이 겪은 실패 방식이기 때문입니다(크론 하나가 5일간 조용히 실패한 적이
있습니다). 그런데 수동으로 두었더니 **감사 자체가 나흘간 멈춰 있었고 아무도
몰랐습니다.** 그래서 매일 돌리되, Slack 으로는 월요일에 한 번만 보냅니다.
매일 통계를 보내면 소음이 되고, 소음이 되면 사람이 알림을 끕니다.
감사가 실패하면 그때는 바로 부릅니다.
...
Slack   ✅ 승인 · 🔴 변경 요청 · 💬 의견 — [레포] PR #번호 — <링크>
        ⏸ 리뷰 못 함 (체크아웃 실패·낡은 큐) — 판정과 구분됩니다
```
...
- **대상 커밋 고정** — 워크트리는 베이스 클론의 HEAD 에서 만들어지므로 PR 커밋이 아닐 수 있습니다.
  지시문이 목표 SHA 로 체크아웃하는 명령을 그대로 담고 있어서, 리뷰어는 항상 그 커밋을 봅니다.
...
- **오탐 신고** — 요약 코멘트 끝에 👍/👎 반응을 요청하고, 감사가 매일 그것을 읽어
  DB 에 쌓습니다. 오탐률은 자동 판정이 불가능해서 사람 라벨링 말고 방법이 없습니다.
  반응은 요약 코멘트에 적힌 head SHA 가 맞는 실행에만 귀속됩니다 — 코멘트 하나를
  계속 고치므로, 그러지 않으면 같은 반응이 여러 실행에 중복으로 세어집니다.
- **판정과 보류의 구분** — 차단급 문제를 찾은 것은 리뷰를 **마친** 것이라 `변경 요청`
  입니다. `보류` 는 리뷰를 **하지 못했을 때**뿐입니다(워크트리 체크아웃 실패, 낡은 큐).
  전에는 둘이 섞여서, 리뷰를 다 하고 문제를 찾은 것이 Slack 에 "리뷰 보류" 로 나갔고
  팀은 그것을 "리뷰가 멈췄다" 로 읽었습니다. 정반대입니다.
...
│   ├── hermes-review-audit.py       리뷰 계약 준수 감사 + 오탐 신고 수집
│   ├── hermes-kanban-restore.py     손상된 칸반 보드 복구
